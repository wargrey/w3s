...
(geo-hb-append #:gapsize (+ (box-vertical-margin $root:n) (box-vertical-margin $root:s))
               (geo-frame #:border (desc-border #:color (current-css-element-color) #:width 1 #:style 'dotted)
                          (geo-frame #:margin (box-vertical-margin $root:n) #:padding (box-vertical-inset $root:n)
                                     #:border (box-border $root:n) #:background (box-brush $root:n)
                                     (geo-paragraph #:color (box-color $root:n) #:max-width (box-width $root:n)
                                                    (pretty-format $root:n) (box-font $root:n))))
               (geo-frame #:border (desc-border #:color (current-css-element-color) #:width 1 #:style 'dotted)
                          (geo-frame #:margin (box-vertical-margin $root:s) #:padding (box-vertical-inset $root:s)
                                     #:border (box-border $root:s) #:background (box-brush $root:s)
                                     (geo-paragraph #:color (box-color $root:s) #:max-width (box-width $root:s) 
