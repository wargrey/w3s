...
(define #:forall (seed) make-xml-event-handler/fold : (-> (Listof (XML-Event-Handlerof seed)) (XML-Event-Handlerof seed))
  (lambda [handlers]
    (xml-event-handler (λ [[pname : (U String Symbol)] [version : Nonnegative-Flonum] [encoding : (Option String)]
                                                       [standalone? : Boolean] [etype? : Boolean] [datum0 : seed]] : (Option seed)
                         ...
                         (let fold-chain ([datum : seed datum0]
                           ...
                           (cond [(null? handlers) datum]
                                 [else (let ([datum++ ((xml-event-handler-doctype (car handlers)) ?name public system datum)])
                                         (fold-chain (or datum++ datum) (cdr handlers)))])))
                       (λ [[?element : (Option Symbol)] [target : Symbol] [body : (Option String)] [datum0 : seed]] : (Option seed)
                         (let fold-chain ([datum : seed datum0]
                                          [handlers : (Listof (XML-Event-Handlerof seed)) handlers])
                           (cond [(null? handlers) datum]
                                 [else (let ([datum++ ((xml-event-handler-pi (car handlers)) ?element target body datum)])
                                         (fold-chain (or datum++ datum) (cdr handlers)))])))
                       (λ [[name : Symbol] [depth : Index] [attrs : (Option SAX-Attributes)] [empty? : Boolean] [preserve? : Boolean] [datum0 : seed]] : (Option seed)
                         (let fold-chain ([datum : seed datum0]
                                          [handlers : (Listof (XML-Event-Handlerof seed)) handlers])
                           (cond [(null? handlers) datum]
                                 [else (let ([datum++ ((xml-event-handler-element (car handlers)) name depth attrs empty? preserve? datum)])
                                         (fold-chain (or datum++ datum) (cdr handlers)))])))
                       (λ [[element : Symbol] [depth : Index] [pcdata : String] [preserve? : Boolean] [cdata? : Boolean] [datum0 : seed]] : (Option seed)
                         (let fold-chain ([datum : seed datum0]
                                          [handlers : (Listof (XML-Event-Handlerof seed)) handlers])
                           (cond [(null? handlers) datum]
                                 [else (let ([datum++ ((xml-event-handler-pcdata (car handlers)) element depth pcdata preserve? cdata? datum)])
                                         (fold-chain (or datum++ datum) (cdr handlers)))])))
                         ...
                         (let fold-chain ([datum : seed datum0]
                           ...
                           (cond [(null? handlers) datum]
                                 [else (let ([datum++ ((xml-event-handler-geref (car handlers)) entity ?default-char datum)])
                                         (fold-chain (or datum++ datum) (cdr handlers)))])))
                       (λ [[?element : (Option Symbol)] [comment : String] [preserve? : Boolean] [datum0 : seed]] : (Option seed)
                         (let fold-chain ([datum : seed datum0]
                                          [handlers : (Listof (XML-Event-Handlerof seed)) handlers])
                           (cond [(null? handlers) datum]
                                 [else (let ([datum++ ((xml-event-handler-comment (car handlers)) ?element comment preserve? datum)])
                                         (fold-chain (or datum++ datum) (cdr handlers)))]))))))
    ...
    (if (or prefilter postfilter)
        (xml-event-handler (λ [[pname : (U String Symbol)] [version : Nonnegative-Flonum] [encoding : (Option String)]
                           ...
                           (λ [[?name : (Option Symbol)] [public : (Option String)] [system : (Option String)] [datum0 : seed]] : (Option seed)
                             ...
                             datum)
                           (λ [[?element : (Option Symbol)] [target : Symbol] [body : (Option String)] [datum0 : seed]] : (Option seed)
                             (when (and prefilter) ((xml-event-handler-pi prefilter) ?element target body (void)))
                             (define datum : (Option seed) ((xml-event-handler-pi handler) ?element target body datum0))
                             (when (and (not datum) postfilter) ((xml-event-handler-pi postfilter) ?element target body (void)))
                             datum)
                           (λ [[name : Symbol] [depth : Index] [attrs : (Option SAX-Attributes)] [empty? : Boolean] [preserve? : Boolean] [datum0 : seed]] : (Option seed)
                             (when (and prefilter) ((xml-event-handler-element prefilter) name depth attrs empty? preserve? (void)))
                             (define datum : (Option seed) ((xml-event-handler-element handler) name depth attrs empty? preserve? datum0))
                             (when (and (not datum) postfilter) ((xml-event-handler-element postfilter) name depth attrs empty? preserve? (void)))
                             datum)
                           (λ [[element : Symbol] [depth : Index] [pcdata : String] [preserve? : Boolean] [cdata? : Boolean] [datum0 : seed]] : (Option seed)
                             (when (and prefilter) ((xml-event-handler-pcdata prefilter) element depth pcdata preserve? cdata? (void)))
                             (define datum : (Option seed) ((xml-event-handler-pcdata handler) element depth pcdata preserve? cdata? datum0))
                             (when (and (not datum) postfilter) ((xml-event-handler-pcdata postfilter) element depth pcdata preserve? cdata? (void)))
                             datum)
                           (λ [[entity : (U Symbol Index)] [?default-char : (Option Char)] [datum0 : seed]] : (Option seed)
                             ...
                             datum)
                           (λ [[?element : (Option Symbol)] [comment : String] [preserve? : Boolean] [datum0 : seed]] : (Option seed)
                             (when (and prefilter) ((xml-event-handler-comment prefilter) ?element comment preserve? (void)))
                             (define datum : (Option seed) ((xml-event-handler-comment handler) ?element comment preserve? datum0))
                             (when (and (not datum) postfilter) ((xml-event-handler-comment postfilter) ?element comment preserve? (void)))
                             datum))
        handler)))
        ...
        (cond [(eof-object? self) (or (fprolog pname version encoding standalone? #false datum) datum)]
              ...
              [(eq? self <?)
               (let-values ([(p consume++++ scope++++) (xml-sax-pi /dev/xmlin consume++ scope++)])
                 (sax consume++++ scope++++ (or (and (mpair? p) (fprocess #false (mcar p) (mcdr p) datum)) datum)))]
              [(eq? self #\<)
              ...
              [(xml-comment? self) (sax consume++ scope++ (or (fcomment #false (xml-white-space-raw self) (eq? xml:space 'preserve) datum) datum))]
              [else (sax consume++ scope++ datum)])))))
    ...
    (cond [(eof-object? self) (values consume++ scope++ datum0)]
          [else (let* ([?tagname (and (symbol? self) self)]
                  ...
                  (let*-values ([(?attrs empty? consume++++ scope++++ lang space) (xml-sax-element-attributes /dev/xmlin consume++ scope++ xml:lang xml:space)]
                                [(datum) (and ?tagname ?attrs datum0 (or (felement ?tagname (assert scope++ index?) ?attrs empty? (eq? space 'preserve) datum0) datum0))])
      ...
      (cond [(eof-object? rest) (values consume++ scope++ datum)]
            [(xml-comment? self) ;; TODO: dealing with newlines
             (sax-subelement consume++ scope++
                             (and tagname datum
                                  (let ([datum++ (sax-cdata-process tagname atadc fpcdata depth preserve? datum)])
                                    (or (fcomment tagname (xml-white-space-raw self) preserve? datum++) datum++)))
            ...
            [(eq? self #\<)
             (let*-values ([(pcdatum) (and tagname datum (sax-cdata-process tagname atadc fpcdata depth preserve? datum))]
            ...
            [(eq? self </)
             (let*-values ([(pcdatum) (and tagname datum (sax-cdata-process tagname atadc fpcdata depth preserve? datum #true))]
               ...
               (values consume** scope**
                       (and pcdatum (eq? tagname ?name)
                            ...
                            (or (felement ?name (assert scope++ index?) #false #false preserve? pcdatum) pcdatum))))]
            [(eq? self <?)
             (let*-values ([(pcdatum) (and tagname datum (sax-cdata-process tagname atadc fpcdata depth preserve? datum))]
                           [(p consume++++ scope++++) (xml-sax-pi /dev/xmlin consume++ scope++)])
               (sax-subelement consume++++ scope++++
                               (cond [(mpair? p) (and tagname pcdatum (or (fprocess tagname (mcar p) (mcdr p) pcdatum) pcdatum))]
                                     [else pcdatum])
                               xml-whitespaces/0))]
            [(eq? self <!&CDATA&)
             (let*-values ([(pcdatum) (and tagname datum (sax-cdata-process tagname atadc fpcdata depth preserve? datum))]
               ...
               (cond [(not (string? ?cdata)) (sax-subelement consume** scope** pcdatum null)]
                     [else (sax-subelement consume** scope**
                                           (and tagname pcdatum
                                                (or (fpcdata tagname depth ?cdata preserve? #true pcdatum) pcdatum))
                                           xml-whitespaces/0)]))]
            [(index? self)
             (sax-subelement consume++ scope++
                             (and tagname datum
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define sax-cdata-process : (All (seed) (->* (Symbol (Listof (U XML-White-Space String)) (XML-PCData-Handler seed) Index Boolean seed) (Boolean) seed))
  (lambda [tagname atadc fpcdata depth preserve? datum [tail? #false]]
