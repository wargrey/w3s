...
(struct (T) XML::Attr::Type*
  ([category-hint : Datum]
   ...
   [token->datum : (XML-Attribute-Value*->Datum (Option T))])
...
(define-syntax (define-xml-attribute-type-datum* stx)
  (syntax-parse stx #:literals [:]
    [(_ [T:expr AT:id] (~optional (~seq #:as category) #:defaults ([category #'#false]))
        (~alt (~optional (~seq #:λstring λstring) #:defaults ([λstring #'xml:attr-datum->value]))
              (~optional (~seq #:datum? datum?) #:defaults ([datum? #'string?]))
              (~optional (~seq #:λdatum λtoken λxexpr) #:defaults ([λtoken #'xml:attr-value*->string]
                                                                   [λxexpr #'xml:attr-value->string])))
        ...)
       ...
       (syntax/loc stx
         (define attr : (XML::Attr::Type* T)
           (unsafe-make-xml::attr::type* (or category 'attr)
                                         ...
                                         λtoken))))]
    [(_ [T:expr AT:id] #:for xtype (~optional (~seq #:as category) #:defaults ([category #'#false]))
        (~alt (~optional (~seq #:λstring λstring) #:defaults ([λstring #'xml:attr-datum->value]))
              (~optional (~seq #:datum? datum?) #:defaults ([datum? #'string?])))
        ...)
     (with-syntax* ([λtoken (format-id #'xtype "xml:attr-value*->~a" (syntax-e #'xtype))]
                    [λxexpr (format-id #'xtype "xml:attr-value->~a" (syntax-e #'xtype))])
       (syntax/loc stx
         (define-xml-attribute-type-datum* [T AT] #:as category
           #:λstring λstring #:datum? datum? #:λdatum λtoken λxexpr)))]
